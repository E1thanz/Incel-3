...
def pre_parse_program(input_file) -> (list, dict, dict):
    # total amount of pages
    available_pages = [page for page in range(128)]
    true_index = 0
    if input_file.readline().strip() != ">0":
        available_pages.remove(0)
    input_file.seek(0)
    ...
    try:
        with open(input_file, 'r') as program_file:
            ...
            with open(os.path.join(dir_path, "assemblyFile.txt"), 'w') as assembly_file:
                ...
                in_page_index = 0
                true_index = 0
                if program_file.readline().strip() != ">0":
                    assembly_file.write(">0\n")
                program_file.seek(0)
